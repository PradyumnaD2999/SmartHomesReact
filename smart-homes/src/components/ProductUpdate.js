...
function ProductUpdate() {
  ...
  const handleAdd = () => {
    ...
    axios.post('http://localhost:8081/updateproduct', {productDetails})
      .then(res => {
        if(res.data === "Error") {
          alert('Could not update product in database.');
  ...
  return (
    <div>
      ...
      <div class="entry">
      <div class='row'>
        <h2 style={{'font-size': '24px;'}}>Update Product</h2>
        <br/><br/>
        <table>
        <tr>
          <td>Category:</td>
        ...
        <tr>
          ...
          <input style={{float: "right"}} class='form-control' type='text' placeholder='Enter Product Company' name='pcompany' value={manufacturer} onChange={(e) => setManufacturer(e.target.value)}/>
		    ...
		    <button onClick={handleAdd} class='button'>Update</button>
      </div>
